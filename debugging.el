(use-package
 ...
 (dap-session-created
  . (lambda (session) (setq lsp-auto-guess-root nil)))
 (dap-terminated . (lambda (session) (setq lsp-auto-guess-root t)))
 :config
 ;; (global-unset-key (kbd "C-x d"))
 (bind-keys* :prefix-map dap-mode-map :prefix "C-x d")
 (bind-keys
  :map
  dap-mode-map
  ("d" . dap-debug)
 ...
 (setq dap-auto-configure-features
       '(sessions locals breakpoints controls expressions)) ;; consider repl
 ...
 (setq
  ...
  (format
   "https://github.com/vadimcn/codelldb/releases/download/v%s/codelldb-linux-x64.vsix"
