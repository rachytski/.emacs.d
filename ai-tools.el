...
(defun write-to-file-tool-fn (path content)
  (let* ((full-path (concat (custom/current-project-root) path)))
    (progn
      (message "tool:write_to_file: path=%S content=%S"
               full-path
               content)
...
(defun get-diffs (diff &optional debug)
  (if debug
      (message "DIFF: %S, len=%d" diff (length diff)))
  (with-temp-buffer
    (insert diff)
    (goto-char (point-min))
    (let* ((max-pos (+ (buffer-size) 1))
      ...
      (while
          (progn
            (let* ((search-pos
                    (or (search-forward search-tag nil "smth")
                        max-pos))
                   (separator-pos
                    (or (search-forward separator-tag nil "smth")
                        (+ max-pos 1 (length separator-tag))))
                   (replace-pos
                    (or (search-forward replace-tag nil "smth")
                        (+ max-pos 1 (length replace-tag))))
                   (search-str
                    (buffer-substring
                     (+ search-pos 1)
                     (- separator-pos (+ (length separator-tag) 1))))
                   (replace-str
                    (buffer-substring
                     (+ separator-pos 1)
                     (- replace-pos (+ (length replace-tag) 1))))
              ...
              (if has-match
                  (progn
                    (if debug
                        (message
                         "has-match=%s finished=%s max-pos=%d search-pos=%d, separator-pos=%d, replace-pos=%d, search-str=%s, replace-str=%s"
                         has-match
                         finished
                         max-pos
                         search-pos
                    ...
                    (setq result
                          (append
                           result
                           (list (list search-str replace-str))))))
              (not finished))))
      result)))
...
(defun execute-command-if-approved
    (command requires_approval &optional debug)
  "Execute command"
  (if debug
      (message "execute_command command=%S requires_approval=%S"
               command
               requires_approval))
...
(defun read-file-tool-fn (path &optional debug)
  (let* ((full-path (concat (custom/current-project-root) path)))
    (progn
      (if debug
          (message "tool:read_file path=%S" full-path))
      (read-file full-path))))

(defun execute-command-tool-fn
    (command requires_approval &optional debug)
  (progn
    (if debug
        (message
         "tool:execute_command: command=%S requires_approval=%s"
...
`completing-read` is used to prompt the user to select one."
  (let*
      ( ;; 1. Parse the JSON string. `json-read-from-string` converts a JSON
    ...
    (if (not (vectorp completion-candidates))
      ...
      (let ((completion-list (append completion-candidates 'list)))
        (completing-read (format "%S" question) completion-list
                         nil
                         t)))))
...
(defun attempt-completion-tool-fn (result command &optional debug)
  (progn
    (if debug
        (message "tool:attempt_completion result=%S, command=%S"
                 result
                 command))
    (if command
        (execute-command-if-approved command :json-false))
...
(defun new-task-tool-fn (context &optional debug)
  (if debug
      (message "tool:not_implemented:new_task context=%S" context))
  nil)

(defun plan-mode-respond-tool-fn (response &optional debug)
  (if debug
      (message "tool:not_implemented:plan_mode_respond response=%S"
               response))
  nil)

(defun load-mcp-documentation-tool-fn (&optional debug)
  (if debug
      (message "tool:not_implemented:load_mcp_documentation"))
  nil)
...
(defun default-system-directive-fn ()
  ...
  (let* ((cpn (persp-current-name))
    ...
    (format-spec
     (read-file (user-init-path "cline-prompt.json"))
     `((?r . ,cpr)
       (?o . user-os-name)
       (?s . user-shell)
       (?h . user-home-dir)))))
...
(defun search-files-tool-fn (path regex file_pattern &optional debug)
  (let* ((full-path (concat (custom/current-project-root) path)))
    (progn
      (if debug
          (message
           "tool:not_implemented:search_files: path=%S regex=%S file_pattern=%S"
           full-path regex file_pattern))
      nil)))
...
(defun list-files-tool-fn (path recursive &optional debug)
  (let* ((full-path (concat (custom/current-project-root) path)))
    (progn
      (if debug
          (message "tool:list_files path=%S, recursive=%s"
                   full-path
                   recursive))
      (if (eq recursive :json-false)
          (mapconcat
           #'identity
           (directory-files-recursively full-path ".*"
                                        t
                                        '(lambda (dirname) nil))
           "\n")
        (mapconcat
         #'identity (directory-files-recursively full-path ".*" t)
         "\n")))))
...
(defun list-code-definition-names-tool-fn (path &optional debug)
  (let* ((full-path (concat (custom/current-project-root) path)))
    (progn
      (if debug
          (message
           "tool:not_implemented:list_code_definition_names: path=%S"
           full-path))
      nil)))

(defun browser-action-tool-fn
    (action url coordinate text &optional debug)
  (progn
    (if debug
        (message
         "tool:not_implemented:browser_action: action=%s url=%S coordinate=%S text=%S"
         action url coordinate text))
    nil))

(defun use-mcp-tool-tool-fn
    (server_name tool_name arguments &optional debug)
  (progn
    (if debug
        (message
         "tool:not_implemented:use_mcp_tool: server_name=%S, tool_name=%S, arguments=%S"
...
(defun access-mcp-resource-tool-fn (server_name url &optional debug)
  (progn
    (if debug
        (message
         "tool:not_implemented:access_mcp_resource: server_name=%S, uri=%S"
         server_name uri))
    nil))

(defun ask-followup-question-tool-fn
    (question options &optional debug)
  (progn
    (if debug
        (message "tool:ask_followup_question: question=%S options=%S"
                 question
                 options))
    (if (eq options nil)
...
(defun cline-setup-tools ()
  ...
  (setq
   ...
   (gptel-make-tool
    ...
    :function
    (lambda (command requires_approval)
      (execute-command-tool-fn command requires_approval))
   ...
   (gptel-make-tool
    ...
    (list
     '(:name
       "path"
       :type string
       :description "(required) The path of the file to read (relative to the current working directory"))
    :category "filesystem"))
  (setq
   write-to-file-tool
   (gptel-make-tool
    :name "write_to_file"
    :function
    (lambda (path content) (write-to-file-tool-fn path content))
    ...
    (list
     '(:name
       "path"
       :type string
       :description "(required) The path of the file to write to (relative to the current working directory)")
     '(:name
       "content"
       :type string
       :description "(required) The content to write to the file. ALWAYS provide the COMPLETE intended content of the file, without any truncation or omissions. You MUST include ALL parts of the file, even if they haven't been modified."))
    :category "filesystem"))
  (setq
   ...
   (gptel-make-tool
    ...
    (list
     '(:name
       "path"
       :type string
       :description "(required) The path of the file to modify (relative to the current working directory)")
     '(:name
       "diff"
       :type string
       :description
       "(required) One or more SEARCH/REPLACE blocks following this exact format:
  \`\`\`
  ------- SEARCH
...
"))
  ...
  (setq
   ...
   (gptel-make-tool
    :name "search_files"
    :function
    (lambda (path regex file_pattern)
      (search-files-tool-fn path regex file_pattern))
    ...
    (list
     '(:name
       "path"
       :type string
       :description "(required) The path of the directory to search in (relative to the current working directory). This directory will be recursively searched.")
     '(:name
       "regex"
       :type string
  ...
  (setq
   ...
   (gptel-make-tool
    :name "list_files"
    :function
    (lambda (path recursive) (list-files-tool-fn path recursive))
    ...
    (list
     '(:name
       "path"
       :type string
       :description "(required) The path of the directory to list contents for (relative to the current working directory)")
     '(:name
       "recursive"
       :type boolean
  ...
  (setq
   ...
   (gptel-make-tool
    ...
    :function
    (lambda (path) (list-code-definition-names-tool-fn path))
    ...
    (list
     '(:name
       "path"
       :type string
       :description "(required) The path of the directory (relative to the current working directory) to list top level source code definitions for."))
    :category "filesystem"))
  (setq
   browser-action-tool
   (gptel-make-tool
    :name "browser_action"
    :function
    (lambda (action url coordinate text)
      (browser-action-tool-fn action url coordinate text))
    :description
...
- text: (optional) Use this for providing the text for the \`type\` action.
    * Example: <text>Hello, world!</text>"
    ...
    (list
     '(:name
       ...
       :type string
       :description
       "(required) The action to perform. The available actions are:
    * launch: Launch a new Puppeteer-controlled browser instance at the specified URL. This **must always be the first action**.
        - Use with the \`url\` parameter to provide the URL.
        - Ensure the URL is valid and includes the appropriate protocol (e.g. http://localhost:3000/page, file:///path/to/file.html, etc.)
    * click: Click at a specific x,y coordinate.
    ...
    * close: Close the Puppeteer-controlled browser instance. This **must always be the final browser action**.
     ...
     '(:name
       ...
       :type string
       :description
       "(optional) Use this for providing the URL for the \`launch\` action.
    * Example: <url>https://example.com</url>")
     '(:name
       "coordinate"
       :type string
       :description
       "(optional) The X and Y coordinates for the \`click\` action. Coordinates should be within the **3440x1440** resolution.
    * Example: <coordinate>450,300</coordinate>")
     '(:name
       "text"
       :type string
       :description
       "(optional) Use this for providing the text for the \`type\` action.
    * Example: <text>Hello, world!</text>"))
    :category "filesystem"))
  (setq
   use-mcp-tool-tool
   (gptel-make-tool
    :name "use_mcp_tool"
    :function
    (lambda (server_name tool_name arguments)
      (use-mcp-tool-tool-fn server_name tool_name arguments))
    ...
    (list
     '(:name
       "server_name"
       :type string
       :description "(required) The name of the MCP server providing the tool")
     '(:name
       "tool_name"
       :type string
       :description "(required) The name of the tool to execute")
     '(:name
       "arguments"
       :type string
       :description "(required) A JSON object containing the tool's input parameters, following the tool's input schema"))
    :category "filesystem"))
  (setq
   ...
   (gptel-make-tool
    ...
    :function
    (lambda (server_name uri)
      (access-mcp-resource-tool-fn server_name uri))
    ...
    (list
     '(:name
       "server_name"
       :type string
       :description "(required) The name of the MCP server providing the resource")
     '(:name
       "uri"
       :type string
       :description "(required) The URI identifying the specific resource to access"))
    :category "filesystem"))
  (setq
   ...
   (gptel-make-tool
    ...
    (lambda (question options)
      (ask-followup-question-tool-fn question options))
  ...
  (setq
   ...
   (gptel-make-tool
    ...
    (lambda (result command)
      (attempt-completion-tool-fn result command))
...
"
    ...
    (list
     '(:name
       ...
       :type string
       :description
       "(required) The context to preload the new task with. If applicable based on the current task, this should include:
   ...
   (gptel-make-tool
    ...
    (list
     '(:name
       "response"
       :type string
       :description "(required) The response to provide to the user. Do not try to use tools in this parameter, this is simply a chat response. (You MUST use the response parameter, do not simply place the response text directly within <plan_mode_respond> tags.)"))
    :category "filesystem"))
  (setq
   load-mcp-documentation-tool
   (gptel-make-tool
